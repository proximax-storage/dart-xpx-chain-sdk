...
class EncryptedMessage extends Message {
  ...
  static Future<EncryptedMessage> create(String plainTextMessage, String senderPrivateKey, String recipientPublicKey,
    ...
    final encryptedPayload = await crypto.encryptMessage(
        plainTextMessage, HexUtils.getBytes(senderPrivateKey), HexUtils.tryGetBytes(recipientPublicKey)!, isHexMessage);
  ...
  static EncryptedMessage fromPayload(final String payload) {
    if (!HexUtils.isHex(payload)) {
      throw ArgumentError('message payload is not a valid hex string');
    }

    return EncryptedMessage._(payload);
  }

  /// Decrypts an [encryptedMessage] sent for the [NetworkType].
  ///.
  /// Returns a UTF-8 formatted string.
  static Future<PlainMessage> decrypt(
      EncryptedMessage encryptedMessage, String recipientPrivateKey, String senderPublicKey,
      [isHexMessage = false]) async {
    ...
    final decrypted = await crypto.decryptMessage(
        encryptedMessage.payload, HexUtils.hexToBytes(recipientPrivateKey), HexUtils.hexToBytes(senderPublicKey));

    if (isHexMessage) {
      return PlainMessage(payload: ByteUtils.bytesToHex(decrypted));
    }
    return PlainMessage(payload: ByteUtils.bytesToUtf8String(Uint8List.fromList(decrypted)));
  }
