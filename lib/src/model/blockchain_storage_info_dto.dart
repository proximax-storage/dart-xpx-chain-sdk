...
class BlockchainStorageInfoDTO {
  ...
  static List<BlockchainStorageInfoDTO> listFromJson(List<dynamic> json) {
    return json == null ? new List<BlockchainStorageInfoDTO>() : json.map((value) => new BlockchainStorageInfoDTO.fromJson(value)).toList();
  }

  static Map<String, BlockchainStorageInfoDTO> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, BlockchainStorageInfoDTO>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new BlockchainStorageInfoDTO.fromJson(value));
    }
    return map;
  }
}
