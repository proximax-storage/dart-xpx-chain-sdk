...
class NamespaceRoutesApi {
  ...
  Future<NamespaceInfoDTO> getNamespace(String namespaceId) async {
    ...
    String path = "/namespace/{namespaceId}".replaceAll("{format}","json").replaceAll("{" + "namespaceId" + "}", namespaceId.toString());
    ...
    if(response.statusCode >= 400) {
      ...
      return 
          apiClient.deserialize(response.body, 'NamespaceInfoDTO') as NamespaceInfoDTO ;
    } else {
      return null;
    }
  }
  ...
  Future<List<NamespaceInfoDTO>> getNamespacesFromAccount(String accountId, { int pageSize, String id }) async {
    Object postBody = null;
    ...
    // create path and map variables
    String path = "/account/{accountId}/namespaces".replaceAll("{format}","json").replaceAll("{" + "accountId" + "}", accountId.toString());
    ...
    if(response.statusCode >= 400) {
      ...
      return 
        (apiClient.deserialize(response.body, 'List<NamespaceInfoDTO>') as List).map((item) => item as NamespaceInfoDTO).toList();
    } else {
      return null;
    }
  }
  ...
  Future<List<NamespaceInfoDTO>> getNamespacesFromAccounts(Addresses addresses, { int pageSize, String id }) async {
  ...
  Future<List<NamespaceNameDTO>> getNamespacesNames(NamespaceIds namespaceIds) async {
    ...
    if(response.statusCode >= 400) {
      ...
      return 
        (apiClient.deserialize(response.body, 'List<NamespaceNameDTO>') as List).map((item) => item as NamespaceNameDTO).toList();
    } else {
      return null;
    }
  }
